...
│                     GITHUB (Code Storage)                   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Repository: council-news-bot                       │   │
│  │  - Python code (main.py, core/, states/)          │   │
│  │  - Config files (docker-compose.yml, crontab)     │   │
│  │  - Documentation (this report, guides, etc.)      │   │
│  └─────────────────────────────────────────────────────┘   │
│                      ⬇ Push to master                       │
│  ┌─────────────────────────────────────────────────────┐   │
│  │         GITHUB ACTIONS (CI/CD Pipeline)             │   │
│  │  1. Test & Lint → Run tests on 3 Python versions  │   │
│  │  2. Deploy to VPS → SSH + rsync + Docker rebuild  │   │
│  │  3. Ops Monitoring → Daily health checks & backups│   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                              ⬇ Deploys to
┌─────────────────────────────────────────────────────────────┐
│              VPS (vps.example.com) - Ubuntu 22.04            │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  DOCKER COMPOSE (Container Orchestra)               │   │
│  │  ┌──────────────────┐  ┌──────────────────┐        │   │
│  │  │  council_db      │  │  council_news_bot│        │   │
│  │  │  (Postgres 15)   │  │  (Python App)    │        │   │
│  │  │  - Articles DB   │←─│  - Scrapers      │        │   │
│  │  │  - Health logs   │  │  - Poster        │        │   │
│  │  │  - Run summaries │  │  - Monitoring    │        │   │
│  │  └──────────────────┘  └──────────────────┘        │   │
│  └─────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  CRON (Task Scheduler) - 20 jobs                   │   │
│  │  - 07:00 UTC: Scrape NSW/VIC/TAS (evening)         │   │
│  │  - 19:00 UTC: Scrape NSW/VIC/TAS (morning)         │   │
│  │  - */10 min: Process posting queue                 │   │
│  │  - 0 * * *: Hourly briefing to Discord            │   │
│  │  - 0 21 * *: Daily briefing to Discord            │   │
│  │  (+ 13 more scraping jobs for other states/times)  │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
       ⬇ Scrapes               ⬇ Posts              ⬇ Reports
┌──────────────┐   ┌──────────────────┐   ┌──────────────────┐
│   537 COUNCIL│   │  BLUESKY (8 accts)│   │    DISCORD       │
│   WEBSITES   │   │  roundupnewsbot* │   │  - Logs channel  │
│              │   │  .bsky.social    │   │  - Alerts channel│
│  - News pages│   │  - Posts articles│   │  - Hourly reports│
│  - RSS feeds │   │  - ~37 posts/hr  │   │  - Daily health  │
│  - Sitemaps  │   │  - Rate limited  │   │  - Error alerts  │
└──────────────┘   └──────────────────┘   └──────────────────┘
       ⬇ Some councils block us                    
┌──────────────────────────────────────────────────────────────┐
│            WEBSHARE PROXY (Rotating IPs)                     │
│  - Gets around IP blocks                                     │
│  - Costs ~$10/month                                          │
│  - Used for ~50 problematic councils                         │
└──────────────────────────────────────────────────────────────┘
```
