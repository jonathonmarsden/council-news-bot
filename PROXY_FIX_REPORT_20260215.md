...
|---|---|---|---|
| **Local (Mac)** | `REDACTED-rotate:...@proxy.example.com:80` | No restriction (rotating endpoint) | ✅ Working |
| **VPS** | `REDACTED:...@proxy.example.com:80` | **IP-whitelisted** | ❌ Blocked for VPS IP |
...
### Why It Failed
1. VPS `.env` configured to use non-rotating proxy endpoint (`REDACTED:...`)
2. VPS IP (vps.example.com) was not whitelisted in Webshare account
...
COUNCIL_BOT_ROTATING_PROXY=http://REDACTED:REDACTED@proxy.example.com:80
```

**After:**
```bash
COUNCIL_BOT_PROXY=http://REDACTED-rotate:REDACTED@proxy.example.com:80
COUNCIL_BOT_ROTATING_PROXY=http://REDACTED-rotate:REDACTED@proxy.example.com:80
```
