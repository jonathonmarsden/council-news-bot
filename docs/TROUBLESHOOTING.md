...
2.  **Manual Scrape (Test Mode)**:
    ```bash
    ssh root@170.64.186.16 "cd /opt/council-news-bot && docker compose exec bot python3 main.py --council [id] --scrape-only"
    ```
3.  **Check DB Stats**:
    ```bash
    ssh root@170.64.186.16 "sqlite3 /opt/council-news-bot/data/bot.db 'select count(*) from articles where council_id=\"[id]\";'"
    ```
