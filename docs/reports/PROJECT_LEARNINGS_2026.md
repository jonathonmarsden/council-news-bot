...
## 5. New CMS Discovery (Jan 2026) -> "SparkCMS / Generic ASP"
During the remediation of NT and WA councils, a new shared platform key was identified.

- **Platform Name**: SparkCMS (identified via cookies) / Generic ASP.NET.
- **Prevalence**: Dominant in NT (Katherine, MacDonnell, Tiwi) and present in WA (Albany, Esperance).
- **Signature**:
  - URL structure often .
  - HTML Container: `.module-list .row`
  - Title: `.h4` or `span.h4` inside the row.
  - Date: `.h5` or `span.h5`.
- **WAF Behavior**: These sites consistently return 403 or 0 items to standard requests.
- **Solution**:
  - Must use `curl_scraper` (impersonate chrome).
  - Selectors:
    ```json
    "item_selector": ".module-list .row",
    "title_selector": "span.h4, .h4",
    "date_selector": "span.h5, .h5",
    "link_selector": "a.btn"
    ```
