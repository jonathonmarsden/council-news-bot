...
**As-built notes**: Playwright spike passed in an unprivileged LXC (nesting=1,keyctl=1); DB streamed and verified identical (34,776 articles); exactly-once posting confirmed post-cutover; acceptance test 509/532 on cutover day and 531/531 after the council repair sweep. Deploys became PULL-based (see docs/DEVELOPER_GUIDE.md — GitHub runners can't reach the LAN, so deploy.yml/rollback.yml were retired). Backups: daily in-container pg_dump + nightly Proxmox vzdump. One deviation from plan: found and fixed an unpinned playwright dependency that broke fresh image builds (now pinned to the base image version).
**Owner**: Jonathon Marsden.

## Why migrate

- Reclaim DigitalOcean droplet ($X/month → $0).
- Consolidate infrastructure on the existing home server (small-form-factor machine running Proxmox), alongside other self-hosted services.
- Same backup, snapshot, and SSH conventions as `the sibling infrastructure repo`.
...
- Update GitHub repo secrets:
  ...
  - `VPS_USER` → the non-root deploy user (per existing host convention)
...
- [ ] Pick a CT 104 LAN IP (e.g. `<lan-ip>`) and confirm not in use.
...
- [ ] Run `proxmox/create-lxc.sh` adapted from `the sibling infrastructure repo`. Adjust:
  - CT ID: 104
  - Hostname: `council-bot`
  - LAN IP: chosen above
  - Disk: 16 GB
  - RAM: 6 GB
- [ ] Run `lxc/01-base-setup.sh` from `the sibling infrastructure repo` (non-root sudo user, UFW, fail2ban, UTC).
...
- [ ] Update `DEPLOYMENT.md`, `README.md`, and `docs/DEVELOPER_GUIDE.md` in this repo to reflect Rakali as production.
...
Following the `the sibling infrastructure repo` pattern, post-migration we should consider creating a a companion infrastructure repo containing:

- `proxmox/create-lxc.sh` — CT 104 provisioning
- `lxc/01-base-setup.sh` — adapted from the sibling infrastructure repo
