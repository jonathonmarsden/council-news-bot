...
curl -v -x http://bgytwxqn-rotate:tu6y5apbawbi@p.webshare.io:80 https://httpbin.org/ip
...
sed -i 's/bgytwxqn:/bgytwxqn-rotate:/g' /opt/council-news-bot/.env
...
import requests
proxies = {'https': 'http://bgytwxqn-rotate:tu6y5apbawbi@p.webshare.io:80'}
...
time curl -s -x http://bgytwxqn-rotate:... https://httpbin.org/ip
# Should be <500ms
```
...
2. Try different rotating IP:
   ...
   for i in {1..3}; do
     curl -s -x http://bgytwxqn-rotate:... https://httpbin.org/ip | jq .origin
     sleep 1
   done
   ```
