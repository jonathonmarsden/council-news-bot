...
**Root Cause**: VPS environment configured with IP-restricted proxy endpoint (`REDACTED:...`) unsuitable for cloud deployment.

**Solution**: Updated VPS `.env` to use rotating proxy endpoint (`REDACTED-rotate:...`) which doesn't require IP whitelisting.
...
```bash
ssh root@vps.example.com "cat /opt/council-news-bot/.env | grep PROXY"
→ http://REDACTED:REDACTED@proxy.example.com:80 (NO -rotate suffix)
```

**Step 3: Manual Proxy Test (VPS)**
```bash
curl -x http://REDACTED:REDACTED@proxy.example.com:80 https://httpbin.org/ip
→ HTTP/1.1 407 Proxy Authentication Required
→ "The proxy you are connecting is not in your list." ❌ FAILS
```

**Step 4: Root Cause Identified**
- VPS IP (vps.example.com) not whitelisted for standard endpoint
...
```
REDACTED:REDACTED@proxy.example.com:80
```
...
```
REDACTED-rotate:REDACTED@proxy.example.com:80
```
...
```bash
# Before
COUNCIL_BOT_PROXY=http://REDACTED:REDACTED@proxy.example.com:80

# After  
COUNCIL_BOT_PROXY=http://REDACTED-rotate:REDACTED@proxy.example.com:80
```

### Command
```bash
ssh root@vps.example.com "sed -i 's/REDACTED:/REDACTED-rotate:/g' /opt/council-news-bot/.env"
```
...
- Use rotating endpoint: `...@proxy.example.com:80`
- No IP whitelisting needed
- Endpoint: `REDACTED-rotate:password@proxy.example.com`

For permanent on-premises:
- Use standard endpoint: `...@proxy.example.com:80`  
- Configure IP whitelist
- Endpoint: `REDACTED:password@proxy.example.com`
```
...
if grep -q "proxy.example.com" .env; then
...
|------|--------|-------------|
| `/opt/council-news-bot/.env` | `REDACTED:` → `REDACTED-rotate:` | VPS only |
