...
Live accounts: `@lgnewsvic.bsky.social`, `@lgnewsnsw.bsky.social`, etc.
...
# One set per state:
BLUESKY_HANDLE_VIC=lgnewsvic.bsky.social
...
- Crontab requires manual regeneration twice yearly after DST transitions (April and October)
- No proactive alerting if a scheduled run is missed entirely
- Database backups are script-based, not integrated into CI/CD
...
Cron schedule is generated by `scripts/deployment/generate_crontab.py`. **Regenerate after each DST transition** (April and October in Australia).
