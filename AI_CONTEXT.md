...
### Infrastructure
- **VPS**: DigitalOcean Droplet (170.64.186.16)
- **User**: `root`
...
- **Logs**: `ssh root@170.64.186.16 'cd /opt/council-news-bot && docker compose logs -f --tail=100'`
...
## 🔑 Operational Access
**VPS Access**: The agent currently has SSH access to the production VPS (`170.64.186.16`) via `scripts/deployment/deploy_secrets.py` (which contains `HOST`, `USER`, `PASS`).
