...
def main():
    ...
    print(f"Found {len(councils)} TAS councils.")
    
    results = []
    
    # Run in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_council = {executor.submit(test_council, council): council for council in councils if council.get('enabled', True)}
