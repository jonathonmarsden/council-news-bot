...
3.  **Updates**: To update code, edit locally and run:
    ```bash
    rsync -avz --exclude 'venv' --exclude 'bot.db' --exclude '__pycache__' --exclude '.git' ./ root@170.64.186.16:/root/council-news-bot/
    ssh root@170.64.186.16 "systemctl restart council-news-bot"
    ```
